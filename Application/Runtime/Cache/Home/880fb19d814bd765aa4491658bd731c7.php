...
$(function () {
            ...
            if (rows.length > 0) {
                $.messager.confirm('确定操作', '您正在要删除所选的记录吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type : 'POST',
                            url : '<?php echo U("deletepayinfo_info");?>',
                            data : {
                                ids : ids.join(','),
                            },
                            beforeSend : function () {
    ...
    $('#admin_payinfo_box').datagrid({
        width : 980,
        url : '<?php echo U("readallpay",array('student_id'=>$student_id));?>',
       // url : 'user.php',
        ...
        onAfterEdit : function (rowIndex, rowData, changes) {
            ...
            if (inserted_payinfo.length > 0) {
                url = '<?php echo U("addpayinfo_info",array('student_id'=>$student_id));?>';
                info = '新增';
            }

            //修改用户
            if (updated_payinfo.length > 0) {
                url = '<?php echo U("updatepayinfo_info");?>';
                info = '修改';
            }
